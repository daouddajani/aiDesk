...
- **Tickets list filters** (`/dashboard/tickets`) — everyone (agents, supervisors, admins) defaults to "All tickets"; anyone can toggle to "My tickets" (assigned to them only) via `?mine=me` / back via `?mine=all`. Status tabs show live counts scoped to the mine/date filters but not the active tab, so switching tabs doesn't change the other tabs' numbers. A from/to date range (default: 15 days ago through today) filters `received_at` at the Supabase query level, not client-side.
...
- **Reply-to-existing-ticket matching, including customer replies** — `ingestIncomingEmail()` in `src/lib/ticketIngestion.ts` is what mailbox polling actually calls now (not `createTicketFromEmail()` directly). It looks for a ticket already on the same thread — `graph_conversation_id` on Microsoft, or the incoming message's In-Reply-To/References headers matched against tickets' `source_message_id` on IMAP (`ParsedIncomingMessage.threadRefs` in `src/lib/imapPoll.ts`) — and if found, appends the message as a `ticket_comments` row instead of creating a new ticket; falls back to `createTicketFromEmail()` only when no match exists. app.md (line 39/100) only specified this for an agent replying from their own mailbox (detected via a company-agent email lookup built from `auth.admin.listUsers()`, keyed to `assigned_agent_id`/status auto-advance same as `assignTicket()`+external-comment-reply combined) — matching customer replies too was a direct user request beyond that spec, and required a migration: `ticket_comments.author_id` is now nullable, with `external_author_email`/`external_author_name` populated instead when the reply isn't from a known agent (rendered in the ticket detail Activity feed via `comment.author_id ? agentName : external_author_name/email`). A `ticket_comments.source_message_id` column (unique per ticket, mirroring the existing per-company dedup on `tickets.source_message_id`) prevents the same reply being appended twice across polls. A third, last-resort fallback tier (same `company_id` + `sender_email` + normalized subject, most-recent-first) was added to `findMatchingTicketId()` after production duplicates were traced to `sendGraphReply`'s (`src/lib/sendTicketReply.ts`) `/sendMail` fallback for Cc'd or attachment-carrying agent replies — Exchange assigns that detached message a fresh `conversationId`, so the customer's next reply matches neither the `graph_conversation_id` nor `source_message_id` tiers. Subject comparison strips a leading, possibly-repeated `Re:`/`Fwd:`/`FW:` prefix, and the sender-email side is matched via an escaped `ilike` (a raw one is unsafe — `_` and `%` are legal in email local-parts and are ILIKE wildcards). Known trade-off: a customer reusing an identical subject for a genuinely unrelated new issue gets appended to the old ticket instead of starting a new one.
- **Pasted-image attachment capture + recurring-image dedup** — a base64 `data:` URI image pasted directly into an Outlook (or similar) compose body has no MIME part at all, so it was previously invisible to both Graph's attachments API and `mailparser`, and silently deleted by `stripHtml()`'s tag-stripping. `extractInlineImages()` (`src/lib/extractInlineImages.ts`) regex-extracts these from the raw HTML body before `stripHtml()` runs — wired into both `src/lib/imapPoll.ts` (from `parsed.html`) and `src/lib/microsoftGraph.ts`/`poll-mailboxes/route.ts` (from `m.body.content`) — and merges them into the same `IncomingEmailAttachment[]` list `cid:`-referenced inline images already flow through, tagged `isInline: true`/`contentId: null`. Separately, `persistAttachments()` (`src/lib/ticketIngestion.ts`) now sha256-hashes every inline image's bytes (`attachments.content_hash`, via `crypto`, no new dependency) and skips the upload+insert entirely if that exact hash was already seen for the company (`attachments.company_id`, a denormalized column added purely so this lookup is a flat query, matching this codebase's no-nested-selects convention) — so a signature/logo baked into every outgoing email attaches once and is silently skipped on every repeat, while a genuinely new pasted image always attaches. See `docs/superpowers/specs/2026-08-25-inline-pasted-image-attachments-design.md`.
- **Ticket update notifications for the assigned agent** — until now the only live in-app notification was for brand-new tickets; agents had no way to know a ticket they own got a new internal comment or customer/agent-mailbox reply short of reopening it. A `ticket_notifications` table (`company_id`, `ticket_id`, `agent_id`, `ticket_subject`, `is_internal`, `author_label`, `comment_preview`, `read_at`) is populated by a `SECURITY DEFINER` Postgres trigger (`notify_assigned_agent_on_comment()`, `AFTER INSERT ON ticket_comments`) rather than app code, since `ticket_comments` has two separate insertion call sites — the interactive `addComment` action (RLS-scoped) and `attachReplyToTicket` (service-role, email ingestion) — and a DB trigger is the only place that catches both without duplicating the fan-out logic twice. The trigger looks up the ticket's `assigned_agent_id`, skips entirely if unassigned or if the comment's author *is* the assigned agent (no self-notification), and denormalizes ticket subject / internal flag / author label / a 140-char body preview onto the row so the toast component needs zero follow-up queries. Delivery is dual: `TicketUpdateNotifications.tsx` (mounted in `AppShell` alongside the existing new-ticket toast, same `getSession()`-before-`subscribe()` ordering, Realtime-filtered on `agent_id=eq.<uid>`) fires an instant toast, and `/dashboard/notifications` is a persistent, unpaginated (capped at 100, never self-prunes) per-agent list with explicit "mark as read"/"mark all as read" actions — no auto-mark-on-view. RLS mirrors `reminders`' shape (`agent_id = auth.uid() and company_id = current_user_company_id()`) but adds an `update` policy instead of reminders' delete-only pattern, since rows persist permanently and only `read_at` changes. `ticket_notifications` was added to the `supabase_realtime` publication (same one-time step `tickets` needed). The nav badge (`SidebarNav`'s previously-unused `badge` prop) is a server-computed unread count from `dashboard/layout.tsx`, refreshed per navigation — deliberately not a live client subscription, since only the toast itself needed to be real-time.
...
Known follow-ups: pull the existing schema into local migrations (`supabase db pull` or equivalent) so it's tracked in the repo — a lot of schema (AI columns, `ticket_assignment_log`, `ticket_time_entries`, `ai_usage_log`, the `tickets` realtime publication membership) exists only in the remote DB with nothing in the repo to reproduce it from scratch; the Supabase Auth project setting "Allow new users to sign up" is still enabled and needs to be turned off in the dashboard (no available tool/API does this) to satisfy `app.md`'s login-only requirement; and `OPENAI_API_KEY` lingers in `.env.local` but is referenced nowhere in `src/` (AI keys are per-company via Supabase Vault), so it can be dropped.

The repo is on GitHub (`https://github.com/daouddajani/aiDesk`, `main` branch) and **is** linked and deployed to Vercel: project `ai-desk` (`prj_AtX5WtqigO9LU8MGTz9BaMrKQCsk`) under the `daouddajanis-projects` team, git-connected to the repo so pushes to `main` deploy automatically, production at `https://ai-desk-xi.vercel.app`. Both `vercel.json` crons (`/api/cron/poll-mailboxes` every 2 min, `/api/cron/dispatch-reminders` every min) are registered and running in production — `vercel crons ls` confirms them. Env vars are set for Preview and Production (Supabase, Resend, Microsoft, `CRON_SECRET`); `vercel env ls` is the source of truth. Note: the Vercel **MCP** server's `list_projects`/`get_project` only see a subset of this team's projects and 404 on `ai-desk` — use the `vercel` CLI for anything project-scoped here, not the MCP tools.
