...
- **Tickets list filters** (`/dashboard/tickets`) — agents default to "My tickets" (assigned to them only), admins default to "All tickets", either can toggle (`?mine=me` / `?mine=all`). Status tabs show live counts scoped to the mine/date filters but not the active tab, so switching tabs doesn't change the other tabs' numbers. A from/to date range (default: Monday of the current week through today) filters `received_at` at the Supabase query level, not client-side.
